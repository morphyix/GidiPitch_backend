...
- **MANDATORY BREVITY & VALUE DENSITY:** All bullets must be aggressively brief, concise, and persuasive (MAX 30 WORDS per bullet to capture 80% of value). Maximum of 3 main bullet points per slide, unless explicitly noted.
...
const pitchDeckSlidePrompt = (startupData) => {
    ...
    const slides = {
        ...
        cover: baseSlidePrompt(
            "cover",
            `${startupName}`,
            `Generate a single, **hyper-compelling and creative tagline** (MAX 10 WORDS) that immediately sells the **massive opportunity**. Example style: 'Uber: The NetJets of Limos'. MUST reference "${startupName}", "${industry}", and the core value of "${solutions}". (MANDATORY: Return EXACTLY ONE bullet point)`,
        ...
        problem: baseSlidePrompt(
            "problem",
            "Problem: The Multi-Billion Dollar Friction Point",
            `3 bullet points: Each must be a **specific, quantified problem statement** (MAX 30 WORDS each, e.g., "$500B wasted annually" or "80% user drop-off") derived from the "CORE PROBLEM" context. The language must be aggressive, highlighting the **massive financial or efficiency impact** this problem creates.`,
        ...
        market: baseSlidePrompt(
            "market",
            "Market Opportunity: The $Billion Land Grab",
            `3 bullet points: MAX 30 WORDS each. 1. **Quantified TAM** estimate for the "${industry}" context. 2. **Quantified SOM** focused on ${scope || "target geography"}. 3. The **CAGR or key quantified trend** driving market expansion. **Use external research if internal context is vague.**`,
            `Explain the market dynamics and identify the specific beachhead market where "${startupName}" will focus for the next 30 months.`,
            "title-bullets",
        ...
        businessModel: baseSlidePrompt(
            ...
            `3 bullet points: MAX 30 WORDS each. 1. Primary **revenue stream** (from "${businessModel}") and **ASP**. 2. Clear statement on **Unit Economics** (e.g., "LTV $X / CAC $Y = Ratio Z > 3.0"). 3. Key channel for margin defense/cost reduction. **Must be consistent with the solution's nature in the context.**`,
        ...
        goMarket: baseSlidePrompt(
            "goMarket",
            "Go-to-Market (GTM): Viral, Scalable Distribution",
            `3 bullet points (MAX 30 WORDS each) outlining the **three most efficient/scalable customer acquisition channels** (e.g., SEO, Referral, Virality). Include a mention of the estimated CAC or conversion rate. **MUST reference "${businessModel}" and "${features}" for channel choice, prioritizing organic/viral loops.**`,
        ...
        competitions: baseSlidePrompt(
            ...
            `3 bullet points: MAX 30 WORDS each. 1. Identification of **2-3 closest competitors** (from "${competitions}"). 2. Clear statement on "${startupName}'s" **unique, sustainable competitive advantage (Moat)** derived from the "CORE SOLUTION" in the context. 3. Key difference **quantified** (e.g., "5x faster" or "70% lower cost").`,
        ...
        team: baseSlidePrompt(
            "team",
            "Team: The Only Group Built to Win This Market",
            `Generate **one high-impact bullet point per core team member** (MAX 30 WORDS each, max ${teamImageCount} bullets). Each bullet must state the team member's **Name, Role, and a single, quantified career achievement/expertise summary** directly relevant to the startup's success (e.g., "Jane Doe, CTO: 10+ years scaling $100M infra"). MUST be derived from the context data or essential expertise for the ${industry} industry.`,
            `The note must be a single, persuasive paragraph (max 3 sentences) that emphasizes the team's collective domain expertise, founder-market fit, and track record of collaboration/resilience. It must sell the team as capable of executing the plan based on their cumulative skills.`,
            "image-text",
        ...
        // 30. Vision & Exit Strategy - *NEW REQUIRED SLIDE*
        vision: baseSlidePrompt(
            "vision",
            "Vision: The $1B+ Exit Strategy",
            `3 bullet points: MAX 30 WORDS each. 1. The **long-term vision** for market disruption. 2. **2-3 credible strategic acquirers** (e.g., Google, Stripe, Pfizer). 3. The **specific, measurable outcome** (e.g., $100M ARR, 5M users) that makes the company a target for acquisition at a $1B+ valuation. **Use external research to justify potential acquirers/valuations for the ${industry} sector.**`,
        ...
        industrySpecific: baseSlidePrompt(
            ...
            `3 bullet points: MAX 30 WORDS each. Outlining **2 critical derisking factors** (e.g., partnership, IP, compliance status) and **1 key strategic asset** (e.g., proprietary data, regulatory advantage). **MUST use specific details from "${moreInfo}"**.`,
        ...
        ask: baseSlidePrompt(
            ...
            `3 bullet points: MAX 30 WORDS each. 1. **Total Funding Amount** and **Round Type** (from "ALL KEY FINANCIALS, MILESTONES, AND ASK DETAILS" context). 2. The **clear breakdown** of funds use (e.g., "70% Product, 30% GTM"). 3. The **single biggest milestone** the fund will achieve (from "ALL KEY FINANCIALS, MILESTONES, AND ASK DETAILS" context).`,
        ...
        contact: baseSlidePrompt(
            ...
            `2 bullet points: MAX 30 WORDS each. 1. **Clear Call-to-Action** for the investor (e.g., "Schedule a Demo" or "Discuss Term Sheet"). 2. Key contact information (Email and Website).`,
    ...
    if (industryLower === "healthtech" || industryLower === "biotech") {
        ...
        slides.validation = baseSlidePrompt(
            ...
            `3 bullet points: MAX 30 WORDS each. 1. **Strongest quantified clinical/pilot outcome** (e.g., "Phase 2 Trial showed 85% efficacy"). 2. **IP protection status** (e.g., "Patent-pending U.S. priority"). 3. **Source of validation data**. **MUST use specific validation and IP data from "${moreInfo}" or relevant external research for validation.**`,
    ...
    if (industryLower === "fintech") {
        ...
        slides.regulation = baseSlidePrompt(
            ...
            `3 bullet points: MAX 30 WORDS each. 1. **Specific Licensing Status** in ${scope}. 2. **Next compliance milestone** (e.g., "Achieve full regulatory status in Brazil by Q2"). 3. **Regulatory advantage** over competitors. **MUST use specific licensing data from "${moreInfo}" or relevant external research for validation.**`,
    ...
    if (industryLower === "edtech") {
        ...
        slides.outcomes = baseSlidePrompt(
            ...
            `3 bullet points: MAX 30 WORDS each. 1. **Quantifiable student impact result** (e.g., "20% increase in national test scores"). 2. **Alignment with key curriculum/standards**. 3. **Proprietary learning theory/data advantage**. **MUST use specific outcome and curriculum data from "${moreInfo}" or relevant external research for validation.**`,
...
// ... (Rest of the code remains the same as no further modification was requested or is necessary)
